...
export default async function ReferentialsPage() {
  const [departments, categories, methods, settings] = await Promise.all([
    ...
    api<Method[]>('/inspection-methods'),
    api<Record<string, unknown>>('/settings'),
  ]);

  const rules: Array<{ key: string; label: string; format?: (v: unknown) => string }> = [
    { key: 'expense.submissionDeadlineDay', label: 'Saisie des frais au plus tard le', format: (v) => `${v} du mois suivant` },
    { key: 'expense.managerDeadlineWorkingDays', label: 'Validation N+1 sous', format: (v) => `${v} jours ouvrés` },
    { key: 'expense.hrDeadlineWorkingDays', label: 'Contrôle RH / contrôle de gestion sous', format: (v) => `${v} jours ouvrés` },
    { key: 'expense.paymentDay', label: 'Virement des frais le', format: (v) => `${v} du mois` },
    { key: 'expense.travelAllowanceMinKm', label: 'Indemnité de déplacement au-delà de', format: (v) => `${v} km du siège` },
    { key: 'report.dueWorkingDays', label: 'Délai de remise de rapport', format: (v) => `${v} jours ouvrés` },
    { key: 'report.qualityTargetComplaintsPerQuarter', label: 'Réclamations client tolérées', format: (v) => `moins de ${v} par trimestre` },
    { key: 'affair.marginAlertThresholdPoints', label: 'Alerte de marge à partir de', format: (v) => `${v} points d’écart` },
    { key: 'certification.alertDaysBefore', label: 'Alerte certification', format: (v) => `${v} jours avant expiration` },
    { key: 'calibration.alertDaysBefore', label: 'Alerte étalonnage', format: (v) => `${v} jours avant échéance` },
    { key: 'timesheet.unassignedAlertConsecutiveDays', label: 'Alerte inspecteur non affecté après', format: (v) => `${v} jours consécutifs` },
  ];

  const rejectReasons = (settings['expense.rejectReasons'] as string[] | undefined) ?? [];
  ...
  return (
    <>
      ...
      <div className="mt-5">
        <Card title={`Catégories de frais et plafonds — ${categories.length}`}>
          <DataTable>
            <thead>
              <tr>
                <Th>Code</Th>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        ...
        <Card title="Motifs de rejet d’une note de frais">
          ...
          <p className="border-t border-border px-5 py-3 text-[13px] text-subtle">
            Liste fermée : un rejet doit toujours porter un motif issu de la procédure.
          </p>
        </Card>
      </div>
    </>
