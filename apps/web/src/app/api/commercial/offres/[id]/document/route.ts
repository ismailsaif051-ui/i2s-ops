...
/**
 * Relaie le document PDF de l'offre.
 *
...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const upstream = await fetch(`${API_URL}/offers/${id}/document`, {
    headers: { Authorization: `Bearer ${token}` },
    cache: 'no-store',
  });

  if (!upstream.ok) {
    const payload = await upstream.json().catch(() => ({ message: 'Document indisponible.' }));
